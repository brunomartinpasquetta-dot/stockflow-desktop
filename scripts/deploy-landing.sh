...
# DNS saneado (2-sep-2026): el apex ya resuelve solo a este VPS. El --resolve
# queda como blindaje determinístico (inmune a caches DNS viejos del runner).
RES="--resolve bpsgsistemas.com:443:187.127.20.131"
...
# CTAs trackeados esperados: 9 = whatsapp×6 (nav ghost + nav ícono + precio +
# rail FAQ + cierre + mbar) + download-win×2 (hero + precio) + download-mac×1.
# (El botón verde de WhatsApp del hero se eliminó a pedido de Bruno, sep-2026.)
...
echo "   data-sf-cta en el HTML servido: $CTAS (esperados: 9)"
echo "   js/tracking.js → HTTP $JS"
if [ "$PIXEL" -ge 1 ] && [ "$JS" = "200" ] && [ "$CTAS" = "9" ]; then
