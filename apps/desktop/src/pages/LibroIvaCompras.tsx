...
import { exportVatBookPurchasesToExcel } from '@/lib/excelExport'
...
function sumActive(rows: VatBookPurchaseRowDTO[], field: keyof VatBookPurchaseRowDTO): string {
  let s = 0
  for (const r of rows) {
...
export function LibroIvaCompras() {
  ...
  const query = useVatBookPurchases(applied ?? { from: 0, to: 0 }, applied != null)
  ...
  const data = supplierId
    ? allData.filter((r) => {
        const sup = suppliersQuery.data?.find((s) => s.id === supplierId)
        return sup ? r.supplierName === sup.name : true
      })
    : allData
  ...
  function onExcel(): void {
    ...
    exportVatBookPurchasesToExcel(data, { from: applied.from, to: applied.to }, companyQuery.data?.name ?? 'Empresa')
  }
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-lg font-semibold">Libro IVA Compras</h1>
      </div>
              ...
              <TableBody>
                {query.isLoading ? (
                  ...
                  data.map((r) => {
                    ...
                    return (
                      <TableRow key={r.purchaseId} className={voided ? 'line-through opacity-60' : ''}>
                        <TableCell>{formatDate(r.date)}</TableCell>
                        <TableCell>{r.type}</TableCell>
                        <TableCell>{r.supplierInvoiceNumber || '—'}</TableCell>
                        <TableCell>{r.supplierName}</TableCell>
                        <TableCell>{r.supplierCuit ?? ''}</TableCell>
